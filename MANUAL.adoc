...
The type may be primitive such as `int` or `boolean`, an object type like `java.lang.Object`, or `void`.
...
The return type of this method is `TypeKind`, an enumeration which contains constants for each of the eight primitive types, one for object types, and one for `void`.
...
* Primitive wrapper type instances (e.g. `Long`, `Double`)
* `enum` constants (as <<jdk-const,`Constable`>> instances)
* Strings
* Classes (as `Class<?>` or as `ClassDesc`)
* `null`
* The value of a static final field
* Lists of other object-typed constants (as an immutable `List`)
* Maps of other object-typed constants (as an immutable `Map`)
