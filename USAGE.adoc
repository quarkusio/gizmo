...
In order to transform the bytecode of an existing class, we need to create a `ClassTransformer` instance.

[source,java]
----
ClassTransformer ct = new ClassTransformer(classVisitor);
// ...do some transformations
ct.close(); <2>
classVisitor.visitEnd();
----
<1> `ClassTransformer` is backed by a `org.objectweb.asm.ClassVisitor` instance.
<2> The `ClassVisitor#close()` method must be always called before `ClassVisitor#visitEnd()`.
...
FieldCreator fc = ct.addField(FieldDescriptor.of("org/acme/Foo", "bar", String.class)); <1>
fc.setModifiers(Opcodes.ACC_PUBLIC | OpCodes.ACC_FINAL);
ct.close(); <2>
----
<1> Use the `FieldCreator` API to configure the new field.
<2> Writes the bytecode to the existing class.
...
// }
MethodCreator transform = ct.addMethod(MethodDescriptor.ofMethod("org/acme/Foo", "transform", String.class, String.class)); <1>
